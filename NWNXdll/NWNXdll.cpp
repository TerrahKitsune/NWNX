...
void * (__cdecl *MallocNWN)( size_t mem ) = (void *(__cdecl *)(size_t))0x0040D550;
void (__cdecl *FreeNWN)( void * mem ) = (void (__cdecl *)(void *))0x0040D560;

void * (__cdecl *MallocExNext)( size_t mem );
void * __cdecl MallocEx( size_t mem ){
	
	//return MallocNWN( mem );
	return MallocExNext( mem );
}
...
DWORD WINAPI Init(LPVOID lpParam) 
{
	...
	fprintf(logFile, "NWN Extender V.2.7-beta5\n");
	...
	BOOL bOk = HookCode( malloc, MallocEx, (PVOID*)&MallocExNext );
	fprintf(logFile, "* Hooked malloc: %s\n", bOk == -1 ? "Success" : "Failure" );
	bOk = HookCode( free, FreeEx, (PVOID*)&FreeExNext );
	fprintf(logFile, "* Hooked free: %s\n", bOk == -1 ? "Success" : "Failure" );
	fflush(logFile);
